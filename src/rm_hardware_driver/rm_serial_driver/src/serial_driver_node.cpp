...
void SerialDriverNode::init() {
  ...
  // Init
  target_frame_ = this->declare_parameter("target_frame", "gimbal");
  ...
  // Publisher
  serial_receive_data_pub_ = this->create_publisher<auto_aim_interfaces::msg::SerialReceiveData>(
    "serial/receive", rclcpp::SensorDataQoS());
  serial_receive_system_data_pub_ = this->create_publisher<rm_interfaces::msg::GameState>(
    "/game_state", rclcpp::QoS(10));
  // TF broadcaster
...
void SerialDriverNode::listenLoop() {
  ...
  rm_interfaces::msg::GameState receive_system_data;
  auto_aim_interfaces::msg::SerialReceiveData receive_data;
  // rm_interfaces::msg::GameState game_state;
  while (rclcpp::ok()) {
    if (protocol_->receive(receive_data)) {
      ...
      // 逐字段复制
      receive_system_data.current_hp = receive_data.gamestate.current_hp;
      receive_system_data.game_progress = receive_data.gamestate.game_progress;
      receive_system_data.stage_remain_time = receive_data.gamestate.stage_remain_time;
      receive_system_data.armor_id = receive_data.gamestate.armor_id;
      receive_system_data.hurt_type = receive_data.gamestate.hurt_type;
      receive_system_data.my_outpost_hp = receive_data.gamestate.my_outpost_hp;
      receive_system_data.enemy_outpost_hp = receive_data.gamestate.enemy_outpost_hp;
      receive_system_data.my_base_hp = receive_data.gamestate.my_base_hp;
      receive_system_data.enemy_base_hp = receive_data.gamestate.enemy_base_hp;
      receive_system_data.projectile_allowance_17mm = receive_data.gamestate.projectile_allowance_17mm;
      receive_system_data.enemy_outpost_occupied = receive_data.gamestate.enemy_outpost_occupied;
      serial_receive_system_data_pub_->publish(receive_system_data);
    ...
    // if (protocol_->receiveSys(receive_system_data))
    // {
    //  serial_receive_system_data_pub_->publish(receive_system_data);
    // }
    // else {
    //   auto error_message = protocol_->getErrorMessage();
    //   error_message = error_message.empty() ? "unknown" : error_message;
    //   FYT_WARN("serial_driver", "Failed to reveive sentry_sys packet! error message :{}", error_message);
    //   std::this_thread::sleep_for(std::chrono::milliseconds(50));
    // }
  }
}
