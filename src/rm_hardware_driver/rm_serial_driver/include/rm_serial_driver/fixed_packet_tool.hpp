...
bool FixedPacketTool<capacity>::checkPacket(uint8_t *buffer, int recv_len) {
  ...
  // 检查帧头，帧尾,
 if ((buffer[0] != 0x78) || (buffer[capacity - 1] != 0x88)) {
    return false;
  }
